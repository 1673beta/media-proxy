...
export function setMediaProxyConfig(setting?: MediaProxyOptions | null) {
	...
	config = {
		...
		allowedPrivateNetworks:
			setting.allowedPrivateNetworks ??
			defaultDownloadConfig.allowedPrivateNetworks,
...
export default function (
	...
	options: MediaProxyOptions | null | undefined,
	done: (err?: Error) => void,
) {
	...
	fastify.get<{
		...
		return await proxyHandler(request, reply).catch((err) =>
...
async function proxyHandler(
	...
	const url =
		"url" in request.query
			? request.query.url
			: request.params.url && "https://" + request.params.url;
	...
	try {
		...
		if ("emoji" in request.query || "avatar" in request.query) {
			...
			image = convertSharpToWebpStream(
				await sharpBmp(file.path, file.mime),
				498,
				422,
			);
